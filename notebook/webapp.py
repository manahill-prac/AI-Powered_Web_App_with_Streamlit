...
# Unzip model if .keras not already extracted
if not os.path.exists("cats_dogs_model.keras") and os.path.exists("cats_dogs_model.keras.zip"):
    with zipfile.ZipFile("cats_dogs_model.keras.zip", 'r') as zip_ref:
        zip_ref.extractall(".")
@st.cache_resource
def load_model():
...
# ----------------- ABOUT -----------------
    ...
    st.write("- Frameworks: TensorFlow + Streamlit")

# ----------------- CREDITS -----------------
elif choice == "Credits":
    st.header("👩‍💻 Credits")
    st.write("Developed by **Manahil** as part of AI/ML learning projects.")
    st.write("Powered by TensorFlow & Streamlit.")
